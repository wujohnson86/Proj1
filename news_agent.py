...
A command-line agent powered by Claude (Anthropic API) that can summarize
and discuss current news headlines using the get_top_headlines tool from
news_mcp_server.py over MCP.

Same pattern as weather_agent.py and crypto_agent.py — just pointed at a
...
async def run_agent():
    ...
    if not os.environ.get("NEWSAPI_KEY"):
        print("Warning: NEWSAPI_KEY is not set — headline lookups will fail "
              "until you export it.\n")
    ...
    server_params = StdioServerParameters(
        ...
        args=[os.path.join(os.path.dirname(__file__), "news_mcp_server.py")],
        env=dict(os.environ),
    )
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            print("Ask me about the news (type 'quit' to exit).\n")
